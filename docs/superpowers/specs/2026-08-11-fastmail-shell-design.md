...
Installed Release builds also carry `get-task-allow`, a side effect of `CODE_SIGN_STYLE = Automatic` — confirmed with `codesign -d --entitlements`. Any local process can therefore attach a debugger to a logged-in mail session, the same threat model as `isInspectable` above and accepted for the same reason on a single-user machine. Unlike `isInspectable` this was never explicitly ratified, so it is written down here rather than rediscovered.
...
The wildcard is broader than it looks. `*` compiles to the regular expression `.*`, which crosses both `/` and `.`. It is implemented twice — once in `ScriptInjector.matches` (Swift, config time) and again in the JS emitted by `guarded()` (runtime) — and both copies share the same weakness: a pattern like `https://*.fastmail.com/*` would let `https://evil.example/x.fastmail.com/` through both gates, because the wildcard is not anchored to a subdomain boundary. The shipped pattern (`https://app.fastmail.com/*`) has no wildcard in authority position, and `NavigationPolicy` is an independent second gate on the loaded host regardless of what the script's own `@match` says, so this is a landmine to document rather than a live bug.

Every injected script must be a self-contained IIFE. `ScriptInjector.guarded()` wraps each injected script's source in `try { … } catch { … }`, so a top-level `'use strict'` is no longer a directive prologue — it becomes an inert string statement — and top-level `let`, `const`, and `class` declarations are scoped to that `try` block, invisible to anything outside it. The shipped script is already an IIFE with its own `'use strict'` inside the function body, so nothing breaks today, but a future script written in Tampermonkey's top-level-scoping style would fail silently. `ScriptInjector.gatedToHost()` wraps `harness.js` in a bare `if (…) { … }` block for the same reason, with the same consequence: top-level `let`, `const`, and `class` declarations in `harness.js` are scoped to that block, invisible outside it. `harness.js` already ships as an IIFE, so this too is dormant risk rather than a live bug.
